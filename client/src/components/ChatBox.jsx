...
const onSubmit = async(e)=>{
  ...
  try {
    ...
    setPrompt('')
    setMessages(prev => [...prev, {type:'user', content:prompt,timestamp:Date.now(), isImage:false}]);
    const endpoint = mode === 'image' ? '/api/message/image' : '/api/message/text';
    const {data} = await axios.post(endpoint,{chatId:selectedChat._id,prompt,isPublished},{headers:{Authorization:token}})
    if(data.success){
